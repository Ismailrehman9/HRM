...
class TicketController extends Controller
{
    ...
    {   
        ...
        $gmanagers = User::where('created_by', '=', \Auth::user()->creatorId())->where('type', '=', 'General Manager')->get();
        ...
        if (\Auth::user()->can('Manage Ticket')) {
            ...
            return view('ticket.index', compact('tickets','countTicket','countOpenTicket','countonholdTicket','countCloseTicket','ticket_arr', 'gmanagers'));
        } else {
    ...
    public function create()
    {
        if (\Auth::user()->can('Create Ticket')) {
            ...
            $managers = User::where('created_by', \Auth::user()->creatorId())
                ->whereIn('type', ['Manager', 'hr', 'IT', 'General Manager'])
                ->get()
                ->pluck('name', 'id');

            return view('ticket.create', compact('employees', 'managers'));
        } else {
    ...
    // Perform any necessary logic, such as fetching data based on the 'id'
    // For demonstration purposes, let's assume you have a 'Status' model
    // and you want to retrieve the status based on the 'id'.
    $status = Ticket::find($id);

    // Prepare a response
    $response = [
        'msg' => 'Status retrieved successfully', // Customize this message
        'status' => $status // Include the retrieved status data
    ];
    ...
    {
        // dd($request);
        // $ticket = Ticket::find($request->tick_id);
        // // dd($ticket);
        // $ticket->statuss = $request->status;
    ...
    public function editt($id)
    {
        ...
        if (\Auth::user()->can('Editt Ticket')) {
            $employees = User::where('created_by', '=', \Auth::user()->creatorId())->where('type', '=', 'employee')->get()->pluck('name', 'id');

            return view('ticket.editt', compact('ticket', 'employees'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
