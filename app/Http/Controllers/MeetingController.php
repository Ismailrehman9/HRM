            ...
            if (Auth::user()->type == 'employee') {
                $current_employee = Employee::where('user_id', '=', \Auth::user()->id)->first();
                 $meetings = LocalMeeting::orderBy('meetings.id', 'desc')
                    ...
                    ->where(function ($q) use ($current_employee) {
                        $q->where('meeting_employees.employee_id', '=', $current_employee->id)
                            ->orWhere('meetings.created_by', '=', $current_employee->user_id)
                            ->orWhere(function ($q) {
