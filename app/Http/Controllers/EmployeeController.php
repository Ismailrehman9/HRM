...
class EmployeeController extends Controller
{
    ...
    public function create()
    {
        if(\Auth::user()->can('Create Employee'))
        {
            ...
            return view('employee.create', compact('employees', 'employeesId', 'departments', 'designations', 'documents', 'branches', 'company_settings', 'newroles'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
        ...
        if(\Auth::user()->can('Create Employee'))
        {
            ...
            if($validator->fails())
            {
             ...
             $role = Role::find($request->role_id);
                ...
                $user = User::create([
                    ...
                    'type' => $role->name, // Assign the role name to 'type'
                    'lang' => 'en',
                ...
                $user->assignRole($role->name); // Assign the role
            ...
            $employee = Employee::create(
                [
                    ...
                    'phone' => $request['Personal'],
                    'official' => $request['Official_Phone'],
                    'emergency' => $request['Emergency_Phone'],
                    ...
                    // 'initial_balance' => $request['initial_balance_'],
                    'initial_balance' => '0',
                    'created_by' => \Auth::user()->creatorId(),
                ]
            );
            ...
            $accountlist->account_name    = $request['account_holder_name']??"";
            $accountlist->initial_balance = '0';
            $accountlist->account_number  = $request['account_number']??"";
            $accountlist->branch_code     = $request['bank_identifier_code']??"";
            $accountlist->bank_branch     = $request['branch_location']??"";
            $accountlist->user_id         = $user->id;
            ...
            if($request->hasFile('document'))
            {
                ...
                $start_Date = explode('-', $request->dob);
                if($currentMonth>=$start_Date[1])
                {
                    
                $dates = $currentYear . '-' . $start_Date[1] . '-' . $start_Date[2];
                }
                else
                {
                    $dates = $nextYear . '-' . $start_Date[1] . '-' . $start_Date[2];
                }
                $event = new Event();
                ...
                $event->title = $request['name'] ."'s".' Birthday';
                $event->start_date = $dates;
                $event->end_date = $dates;
                $event->color = 'event-danger';
                $event->description = 'Birthday of '.$request['name'];
    ...
    public function edit($id)
    {
        ...
        if(\Auth::user()->can('Edit Employee'))
        {
            ...
            return view('employee.edit', compact('employee', 'employeesId', 'branches', 'departments', 'designations', 'documents', 'user', 'roles'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function update(Request $request, $id)
    {
        ...
        if(\Auth::user()->can('Edit Employee'))
        {
            ...
            $event = Event::where('employee_id', $id)->first();
    ...
    public function joiningletterPdf($id)
    {
        ...
        $joiningletter = JoiningLetter::where('lang', $currantLang)
        ...
        return view('employee.template.joiningletterpdf', compact('joiningletter','employees','obj'));
